...
function Lyrics({ song, currentTime, songId, sly }) {
  ...
  useEffect(() => {
    if (lyrics.length < 1) {
      const url = `/lyrics/${song.trackName
        .split(" ")
  ...
  useEffect(() => {
    if (state.lyrics.length > 1) {
      let currentLine = state.lyrics.findIndex(
        (lyrics) => currentTime >= lyrics.start && currentTime < lyrics.end
      );
      ...
      return (
        <div className="mm-lyrics1">
          <div className="lydivsc">
            <span className="lt">
              Ten tekst nie jest jeszcze zsynchronizowany z utworem
            </span>
            {ly != null
              ? state.lyrics.map((list) => {
                  return <span className="lt">{list.lyrics}</span>;
                })
              : ""}
        ...
        <FullScreen handle={handle}>
          <div className="mm-lyrics">
            {translate == false ? (
              <button
                ...
                onClick={() => {
                  if (lyrics.length < 1) {
                    ...
                    const url = `/pl/lyrics/${song.trackName
                      .split(" ")
                      .map((piece) => piece.toLowerCase())
                      .join("-")}.json`;
                    axios
              ...
              <button
                className="button-translate-active"
                id="confdis"
                onClick={() => {
                  if (lyrics.length < 1) {
            ...
            {fullscreen == false ? (
              ...
              >
                ...
                >
                  ...
                  <polygon points="208 48 208 16 16 16 16 208 48 208 48 70.627 208.687 231.313 231.313 208.687 70.627 48 208 48" />
                  <polygon points="464 304 464 441.373 299.313 276.687 276.687 299.313 441.373 464 304 464 304 496 496 496 496 304 464 304" />
                </svg>
              </button>
            ) : (
              <button
                className="button-fullscreen-active"
                onClick={() => {
                  Exit();
                }}
              >
                <svg
                  ...
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <title>Zamknij pełny ekran</title>
                  <polygon points="204 181.372 38.628 16 16 16 16 38.628 181.372 204 44 204 44 236 236 236 236 44 204 44 204 181.372" />
                  <polygon points="326.628 304 464 304 464 272 272 272 272 464 304 464 304 326.628 473.372 496 496 496 496 473.372 326.628 304" />
                </svg>
              </button>
            )}
            <p>©Musiq</p>
              ...
              {ly != null
                ? state.lyrics.map((list) => {
                    if (currentTime >= list.start && currentTime <= list.end) {
                      if (
                        state.lyrics.indexOf(list) == 0 ||
                        state.lyrics.indexOf(list) == 1
                      ) {
                        document.documentElement.style.setProperty(
                          "--top-lyrics",
                          "10%"
                        ...
                        document.documentElement.style.setProperty(
                          "--top-lyrics",
                          "calc(" +
                            -25 * state.lyrics.indexOf(list) +
                            "% + " +
                            -45 * state.lyrics.indexOf(list) +
                            "px + 10%)"
                        );
                      }
                      return (
                        <div>
                          <span className="lt">{list.lyrics}</span>
                        </div>
                      );
                    } else {
                      return (
                        <div>
                          <span className="lt-n">{list.lyrics}</span>
                        </div>
                      );
                    }
                  })
          ...
          {ly != null
            ? state.lyrics.map((list) => {
                if (currentTime >= list.start && currentTime <= list.end) {
                  ...
                  return (
                    <div>
                      <span className="lt-no">{list.lyrics}</span>
                    </div>
                  );
                }
              })
